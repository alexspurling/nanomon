...
void LineGraph::pan(const double delta_idx) {
...
void LineGraph::zoom(const double factor, const double mouse_ratio) {
...
void LineGraph::set_view_window(const double view_start, const double view_end) {
...
void LineGraph::set_view_start(const int start) {
    m_view_start = start;
    m_last_stats.start = start;
}

void LineGraph::set_view_end(const int end) {
    m_view_end = end;
    m_last_stats.end = end;
}

int LineGraph::calculate_step(const int start, const int end) const {
    return std::max(1, (end - start) / m_max_vertices);
}

void LineGraph::add_sample() {
...
void LineGraph::update_scroll(const double sample_progress) {
    const int step = calculate_step(m_view_start, m_view_end);
