...
LineGraph::LineGraph(size_t num_points,
...
void LineGraph::initialise_x_values() {
    ...
    for (size_t i = 0; i < m_num_points; i++) {
        const float x = left_x + static_cast<float>(i) * dx;
        m_data[i * 2] = x;
    }
}

void LineGraph::advance_time(double game_time) {
...
double LineGraph::get_sample_x(int i) const {
    const double t = static_cast<double>(i) / (static_cast<double>(m_num_points) - 2.0);
...
double LineGraph::compute_y(double sample_x) const {
